...
- Finallizing our project for deployment and cleaning up any linter

In regards to our poller bug that had been going on the past 2 days, we tried many different solutions and eventually Matthew and Daniel were able to figure that the bug was in our deployed section of the poller in the gitlab-ci.yml file. We had somewhere in there to be reviews-api instead of reviews-poller. After that, we focused on cleaning up our codes with black and flake8 linters to get rid of any unsued variables as well as any comments and print statements. Everything is working as we expectly and fully functioning.

## September 14,2022 - September 15, 2022

Today we worked on:

- Working on our blocker regarding our poller not working properly in the deployed version

We ran into an issue on our deployed version when trying to create a review even though the development site were fully functioning and working properly. For some reason with the deployed site, we were not able to create a review even though we were logged in and if we clicked on the create a review button again we were redirected to the log in page. We spent pretty much 2 days on this bug and it looked like there were other groups that ran into the same issue as us. As it turned out, it seemed like the UserV0 was not gettng created after submitting the review, which caused the review to not be created and posted.

## September 13, 2022

Today I worked on:

- Fixing up the styling of the footer and the search bar

I had discovered this morning when we started our project time that Kelly and I accidentally hid the scrollbar for the entire application instead of just the reviews list. After some research, it turned out that I just needed to specify the class selector for the review box only and that was able to bring back the scrollbar for the application. I also spent the majority of my day fixing up the styling of the footer so that it can be more condensed and easier to look at. I also fixed up some small styling with the search bar so that the spacing and allignment looked a little bit better.

## September 12, 2022

Today I worked on:

- Writing my unit test and some styling for the list of reviews box

We had our practice assessment today so there was not much time to work on project stuffs. But I was able to write up my unit test for the project. I ran into some issue while trying to test if the test worked. I asked Kelly for some help with clarifying how the JSON input body was supposed to look like and what the response was supposed to be in case I had made any mistakes with the input body. I eventually found out that it was because the url I had used was not completely correct. And after fixing that, the test was able to pass correctly. Kelly and I also worked on styling the list of reviews. We changed the review box background to black and each individidual review section to be white. We also worked on hiding the scrollbar for the reviews list section.

## September 9, 2022

Today we worked on:

- Small fixes with linters and minor styling adjustments

Matthew was able to get the log in issue that we had yesterday fixed up by getting rid of django's authentication method and replacing it with just the DJWTO's authentication's method. Other than that, we had to fix up some more linters issue that was added but wasn't addressed and we didn't see it until the pipelines fail again. For the most part, we sort of working on some minor styling adjustments to our site to make it look better while Nikasha was working on completing the create a review section on the front end.
...
REACT*APP_TMDB_API_KEY and REACT_APP_TMDB_URL were initiated in the docker-compose file. I had, at first, named REACT_APP_TMDB_API_KEY as TMDB_API_KEY only and that was causing a bug for awhile until i realized that I needed to add the REACT_APP* portion in front of it.
